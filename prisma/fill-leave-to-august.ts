...
function isLeaveAppointment(appointment: Pick<AppointmentSnapshot, 'patientName' | 'notes'>) {
...
function createDateAtTime(baseDate: Date, hour: number, minute: number) {
  return new Date(
...
function* eachSlotForDay(day: Date) {
...
function buildPatterns(appointments: AppointmentSnapshot[]) {
  const uniquePatterns = new Map<string, LeavePattern>()

  for (const appointment of appointments) {
    if (!isLeaveAppointment(appointment)) continue

    const pattern: LeavePattern = {
...
async function ensureSlotsExist(resourceIds: number[]) {
...
function canApplyPattern(
  pattern: LeavePattern,
  day: Date,
  appointmentsByKey: Map<string, AppointmentSnapshot>,
) {
...
async function applyPatterns(patterns: LeavePattern[]) {
